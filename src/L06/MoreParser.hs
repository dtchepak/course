...
hex =
  let hexDigits :: Parser String
      hexDigits = is 'u' >> replicateM 4 (satisfy isHexDigit)
      hexToInt :: String -> Int
      hexToInt s = case readHex s of
                    (n,_):_ -> n
                    _       -> 0
  in fmap (chr . hexToInt) hexDigits
...
betweenSepbyComma ::
  ...
  -> Parser a
  -> Parser [a]
betweenSepbyComma l r =
  betweenCharTok l r . flip sepby (charTok ',')
