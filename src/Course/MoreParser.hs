...
betweenCharTok ::
  ...
  -> Parser a
  -> Parser a
betweenCharTok s e p =
  between (charTok s) (charTok e) p
...
hex ::
  Parser Char
hex =
  readHex <$> replicateA 4 (satisfy isHexDigit)
    >>= foldOptional (pure . chr) failed
