...
instance Monad f => Applicative (StateT s f) where
  ...
  f <*> s =
    f >>= (<$> s)
{- Without using StateT monad instance
  StateT sf <*> sa =
    StateT $ \s ->
       sf s >>= (\(f, s') -> runStateT (f <$> sa) s')
-}
...
distinctF ::
  ...
  -> Optional (List a)
distinctF =
  flip evalT S.empty . filtering (\a ->
    StateT (\s -> if a>100 then Empty else Full (a `S.notMember` s, a `S.insert` s))
  )
        ...
        StateT (\s ->
            OptionalT $
                if a > 100 then
                    ...
                    (pure (a `S.notMember` s, a `S.insert` s))
        )
